    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          hint={`Mode: ${gateway.mode}, hash ${gateway.hash}`}
        />
        <StatCard
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr,0.9fr]">
        <Panel>
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-slate-950">Config sections</h3>
              <p className="text-sm text-slate-600">Plain-English summaries of the live OpenClaw config.</p>
            </div>
            <Link href="/config" className="text-sm font-semibold text-sky-700 hover:text-sky-900">
              Open config
            </Link>
          </div>
        ...
        <div className="grid gap-6">
          <Panel>
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold text-slate-950">Top findings</h3>
                <p className="text-sm text-slate-600">Latest security audit output.</p>
              </div>
              <Link href="/audit" className="text-sm font-semibold text-sky-700 hover:text-sky-900">
                Review findings
              </Link>
            </div>
            <div className="space-y-3">
              {audit.findings.slice(0, 3).map((finding) => (
                <div key={finding.findingId} className="rounded-xl border border-slate-200 p-4">
                  <div className="mb-2 flex items-center gap-2">
                  ...
                  <p className="text-sm text-slate-600">{finding.currentState}</p>
                </div>
              ))}
            </div>
          </Panel>
          ...
          <Panel>
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold text-slate-950">Latest suggestions</h3>
                <p className="text-sm text-slate-600">Recommendation cards tailored to the current setup.</p>
              </div>
              <Link href="/suggestions" className="text-sm font-semibold text-sky-700 hover:text-sky-900">
                Open feed
              </Link>
            </div>
            <div className="space-y-3">
